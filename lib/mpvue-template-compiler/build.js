...
function mark(path, options, deps, iteratorArr) {
  ...
  var vModels = options.vModels;

  if (!vModels) {
    vModels = options.vModels = [];
  }
  ...
  if (hasModel && vModels.indexOf(hasModel.value) < 0) {
    vModels.push(hasModel.value);
  }

  if (needEventsID) {
    // var typeName = (path.staticClass || path.tag).replace(/\"/g, '');
    var charsId = (options.scopeId || 'no').split('-').pop() + '-' + deps.eventIndex;
    var eventId = getWxEleId(charsId, currentArr);
  ...
  path['mpcomid'] = value;
  path['mpScope'] = currentArr;
  path['vModels'] = vModels;
  deps.comIndex += 1;
}
...
function transformDynamicClass(staticClass, clsBinding) {
  ...
  var cls = prettier.format(result.code, { semi: false, singleQuote: true, parser: 'babel' }).slice(1).slice(0, -1).replace(/\n|\r/g, '');
...
var component = {
  ...
  convertComponent: function convertComponent(ast, components, slotName, tempaltName) {
    var attrsMap = ast.attrsMap;
    var tag = ast.tag;
    var mpcomid = ast.mpcomid;
    var mpScope = (ast.mpScope || []);
    var vModels = ast.vModels || [];
    var vModelVars = vModels.length > 0 ? ',' + vModels.join(',') : '';
    var mpScopeVars = mpScope.length > 0 ? ',' + mpScope.join(',') : ''
    var slots = ast.slots;
    if (slotName) {
      var vars = slotNameMaps[tempaltName] || '';
      var kName = "$kk+" + mpcomid;
      attrsMap['data'] = "{{...$root[" + kName + "], ...$root[$k],...$root[p], ...$root[0]" + vars + ", $root,$kk:" + kName + "+'_',$k:" + kName + vModelVars + "}}";
      ...
      var kName = "$kk+" + mpcomid;
      // var kkName = mpScopeVars ? kName+"+','":'$kk'
      attrsMap['data'] = "{{...$root[" + kName + "] " + mpScopeVars + ",$kk:" + kName + "+'_',$k:" + kName + ",$root" + restSlotsName + vModelVars + "}}";
      slotNameMaps[components[tag].name] = mpScopeVars;
...
var tag = function (ast, options, component) {
  ...
  if ((ifText || elseif || elseText || forText) && tag === 'template') {
    ...
    ast = component.convertComponent(ast, components, slotName, options.name, options.vModels);
    ast.tag = 'template';
