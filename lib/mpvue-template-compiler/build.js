...
var component = {
  ...
  convertComponent: function convertComponent (ast, components, slotName,tempaltName) {
    ...
    if (slotName) {
      ...
      attrsMap['data'] = "{{...$root[$k], $root,$pk,$k,$kk "+vars+"}}";
      attrsMap['is'] = "{{" + slotName + "}}";
    } else {
      var slotsName = getSlotsName(slots);
      var restSlotsName = slotsName ? (", " + slotsName) : '';
      var kName = "($pk||$kk)+"+mpcomid;
      var kkName = mpScopeVars ? kName+"+','":'$kk'
      attrsMap['data'] = "{{...$root[" + kName + "] "+mpScopeVars+",$pk:($pk? "+kName+"+','"+":"+kkName+") ,$k:" + kName + ",$kk:"+kkName+",$root" + restSlotsName + "}}";
...
var tag = function (ast, options) {
  ...
  if ((ifText || elseif || elseText || forText) && tag === 'template') {
    ...
    ast = component.convertComponent(ast, components, slotName,options.name);
    ast.tag = 'template';
