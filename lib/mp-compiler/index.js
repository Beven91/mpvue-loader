...
const emitFileTimer = null
let slotsHookAdded = false

// 调用 compiler 生成 wxml
function genComponentWxml (compiled, options, emitFile, emitError, emitWarning, resourcePath) {
...
function createWxml (emitWarning, emitError, emitFile, resourcePath, rootComponent, compiled, html) {
  ...
  if (rootComponent) {
    ...
    wxmlContent = genComponentWxml(compiled, options, emitFile, emitError, emitWarning, resourcePath)
...
function compileMPScript (script, mpOptioins, moduleId) {
  ...
  const fileInfo = resolveTarget(this.resourcePath, compilerContext.getOptions().entry)
...
function compileMP (content, mpOptioins) {
  ...
  rc.plugins = (rc.plugins || []).concat([parseConfig])
  ...
  const fileInfo = resolveTarget(resourcePath, compilerOptions.entry)
  ...
  if (isApp || isPage) {
    ...
    if (config) {
      ...
      // 只有 app 才处理 pages
      if (isApp) {
        const pages = Object.keys(compilerOptions.entry).concat(configObj.pages).filter(v => v && v !== 'app').map(getPageSrc)
