...
function compileMPScript(script, mpOptioins, moduleId) {
  ...
  const babelOptions = typeof babelrc === 'string' ? { extends: babelrc, plugins: [parseComponentsDeps] } : babelrc;
  babelOptions.plugins = [parseComponentsDeps];
  if (script.src) { // 处理src
...
function compileMP(content, mpOptioins) {
  ...
  const babelrc = getBabelrc(mpOptioins.globalBabelrc)
  const rc = typeof babelrc === 'string' ? { extends: babelrc, plugins: [parseConfig] } : babelrc;
  rc.plugins = ([parseConfig]);
